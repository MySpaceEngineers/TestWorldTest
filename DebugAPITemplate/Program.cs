...
namespace PB.Lite
{
    ...
    public class Program : MyGridProgram
    {
        ...
        public Program()
        {
            ...
            Debug.PrintChat($"Starting PB, version {version}", Me.CubeGrid.CustomName);
        }
        ...
        public void Main(string argument, UpdateType updateType)
        {
            switch (argument)
            {
                case "ai":
                    {
                        var ai = GridTerminalSystem.GetBlockWithName("AI Flight") as IMyFlightMovementBlock;
                        ai.LookAtPosition = Vector3.Zero;
                        Debug.PrintHUD($"look: {ai.LookAtPosition}", seconds: 3);
                        break;
                    }
                case "point":
                    {
                        var point = Me.WorldMatrix.Translation;
                        Debug.DrawPoint(point, Color.Red, onTop: true);
                        break;
                    }
                case "matrix":
                    {
                        var matrix = Me.WorldMatrix;
                        Debug.DrawMatrix(matrix, onTop: true);
                        break;
                    }
                case "ship":
                    {
                        var matrix = Me.CubeGrid.WorldMatrix;
                        Debug.DrawMatrix(matrix, onTop: true);
                        break;
                    }
                case "CLEAR":
